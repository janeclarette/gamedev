...
const loadBuildings = (scene, buildingLayer) => {
  ...
  const loadAsset = (path, position, rotation = [0, 0, 0], scale = [1, 1, 1], isFBX = false, isOBJ = false, texturePath = null, highlightEdges = false, name = '') => {
    const loader = isFBX ? fbxLoader : (isOBJ ? objLoader : gltfLoader);
    if (!loader) return;
  ...
  layout.forEach(({ type, position, rotation, scale, isFBX, isOBJ, texturePath, highlightEdges, name }) => {
    loadAsset(assets[type], position, rotation, scale, isFBX, isOBJ, texturePath, highlightEdges, name);
  });
