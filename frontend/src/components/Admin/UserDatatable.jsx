...
const UserDataTable = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch("http://localhost:8000/adminusers/get-users");
        if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this user?")) {
      try {
        const response = await fetch(`http://localhost:8000/adminusers/delete-user/${id}`, {
          method: "DELETE",
        });
  ...
  const handleSave = async () => {
    if (selectedUser && window.confirm("Are you sure you want to update this user?")) {
      ...
      try {
        const response = await fetch(`http://localhost:8000/adminusers/update-user/${selectedUser.id}`, {
          method: "PUT",
          body: formData,
        });
  ...
  const handleCreateSave = async () => {
    ...
    try {
      const response = await fetch("http://localhost:8000/adminusers/create-user", {
        method: "POST",
        body: formData,
      });
